...
/**
 * Standard newsroom card — cover image with hover zoom, category chip, meta
 * row, title, excerpt and a "Read more" link to the article. Mirrors the blog
 * card exactly; only the link target differs (`/newsroom/:slug`). Motion only
 * touches transform / opacity so it stays on the GPU compositor.
 */
export function NewsCard({ post, className }: { post: BlogPost; className?: string }) {
