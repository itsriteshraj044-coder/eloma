    ...
    <footer className="border-t border-navy-100 bg-white">
      <Container>
        ...
        <div className="grid gap-10 py-12 sm:gap-12 sm:py-14 xl:grid-cols-[1.4fr_3fr] xl:py-16 3xl:py-20 3xl:gap-14 4xl:py-24 4xl:gap-16">
          <div>
            <Logo />
            <p className="mt-6 text-body-fluid text-navy-500">{BRAND.tagline}</p>

            <div className="mt-8 flex flex-col gap-3 text-body-fluid 3xl:gap-4">
              <a
              ...
              {SOCIAL_LINKS.map((s) => {
                ...
                return (
                  <li key={s.label}>
                    ...
                    >
                      <Icon className="h-4 w-4 3xl:h-5 3xl:w-5 4xl:h-6 4xl:w-6 5xl:h-7 5xl:w-7" aria-hidden="true" />
                    </a>
                  </li>
                );
              })}
          ...
          <nav aria-label="Footer" className="grid grid-cols-2 gap-6 sm:grid-cols-3 sm:gap-8 md:grid-cols-4 xl:grid-cols-5 3xl:gap-10 4xl:gap-14 5xl:gap-16">
            {FOOTER_COLUMNS.map((col) => (
              <div key={col.heading}>
                <h3 className="text-eyebrow-fluid uppercase text-navy-900">
                  {col.heading}
                </h3>
                <ul className="mt-5 flex flex-col gap-3">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="text-body-fluid text-navy-500 transition-colors hover:text-navy-900"
                      >
                        {link.label}
                      </a>
                    </li>
        ...
        {/* Bottom: legal */}
        <div className="flex flex-col gap-4 border-t border-navy-100 py-6 text-body-fluid text-navy-400 md:flex-row md:items-center md:justify-between 3xl:py-8 4xl:py-10">
