...
/* ════════════════════════════════════════════════════════════════════════
   Blog article — full reading view for a single post (`/blog/:slug`). Light
   theme, single emerald accent, generous reading measure. Cover hero, prose
   body with sub-headings and pull-quotes, key takeaways, and related posts.
   ════════════════════════════════════════════════════════════════════════ */
        ...
        <Container>
          <div className="mx-auto max-w-xl text-center">
            ...
            <p className="mt-5 text-body-fluid text-navy-500">
              It may have moved or been retired. Explore the latest insights instead.
            </p>
            <div className="mt-8">
              <Button href="/blog" variant="primary" size="lg" iconRight={<ArrowRight className="h-4 w-4" />}>
                Back to the blog
              </Button>
            </div>
          </div>
...
export default function BlogPost() {
  ...
  const related = useMemo(
    () => (post ? BLOG_POSTS.filter((p) => p.slug !== post.slug).slice(0, 3) : []),
    [post],
  );
  ...
  return (
    <main className="overflow-x-clip">
      ...
      {/* ── Related ───────────────────────────────────────────────────── */}
      {related.length > 0 && (
        <section aria-label="More insights" className="section-py overflow-x-clip bg-navy-50/40">
          <Container>
            <motion.div
              variants={staggerParent(0.1)}
              initial="hidden"
